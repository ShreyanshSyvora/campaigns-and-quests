...
export const authTwitter = async (req: Request, res: Response) => {
  try {
    ...
    if (!role || !["user", "campaignOwner"].includes(role)) {
    ...
    if (role === "user") {
      ...
      updatedDoc = await CampaignOwner.findByIdAndUpdate(
        id,
        { twitter_id: twitterUser.id },
        { new: true }
      );
    ...
    console.error("Error linking twitter:", err.response?.data || err.message);
